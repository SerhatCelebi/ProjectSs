...
public class HardGameManager : MonoBehaviour, ISelectable
{
    ...
    [SerializeField] Text fastNoteText;

    static int hideCount = 46;
    ...
    public void FastNoteButton()
    {
        AudioManager.Instance.Play("Click");
        if (fastNoteCount > 0)
        {
            FastNoteController.Instance.TakeFastNote(allSquares, allObjSquares);
            fastNoteCount--;
            UpdateFastNoteText();
        }
    }
    ...
    void UpdateFastNoteText()
    {
        fastNoteText.text = fastNoteCount.ToString();
    }
